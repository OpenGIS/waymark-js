...
test.describe('2. Instances', () => {
  ...
  test.describe('Accessing the MapLibre instance', () => {
    test('map instance is attached to window.waymarkInstance', async ({ page }) => {
    ...
    test('factory API is attached to window.createWaymarkInstance', async ({ page }) => {
  ...
  test.describe('Initial GeoJSON', () => {
    test('renders initial GeoJSON from the third argument with no console errors', async ({ page }) => {
      const errors = []
      page.on('console', (msg) => {
      ...
      const renderResult = await page.evaluate(async () => {
        ...
        return {
          hasSource: Boolean(instance.map.getSource('waymark-map-geojson-test-geojson-source')),
          hasLayer: Boolean(instance.map.getLayer('waymark-map-geojson-test-geojson-layer')),
        }
      })

      await expect(page.locator('#map-geojson-test canvas')).toBeVisible({ timeout: 5000 })
