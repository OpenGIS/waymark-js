...
`toJSON()` omits runtime-injected default basemaps, preserves explicitly authored basemap entries (including explicit OpenFreeMap values), omits empty basemap arrays, and serialises basemap keys in canonical `raster` then `vector` order.

`toJSON()` reflects live basemap mutations from runtime commands/UI controls (raster opacity changes, raster reorder, vector active selection) so post-mutation serialisation stays in sync with `waymark:map.basemaps.changed` snapshots.
