...
When `data.layers` is provided in `instanceDocument`, Waymark adds one instance-scoped GeoJSON source/layer per entry with `{ geoJSON }` during initial map setup.
