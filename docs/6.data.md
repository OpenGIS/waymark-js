...
When a type has an `icon` property (see [API docs](1.api.md#paint--types)), its point family renders as MapLibre `symbol` layers instead of `circle` layers, using the named icon from the bundled `waymark-icons` sprite with the type's `circle-color` as fill. The `mountedFamilies` payload includes `"symbol"` for such layers.
