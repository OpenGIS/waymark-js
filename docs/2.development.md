...
8. Run the full CI gate locally: `npm run test:ci` (format check, docs sync, unit tests, browser tests).

`npm run test:browser` boots a dedicated Playwright Vite server on `127.0.0.1:4173` with `--strictPort`, so browser tests are deterministic even when your normal dev port is occupied. A second webServer runs `vite preview` on `127.0.0.1:4174` to serve the built `dist/` for the built-bundle test (`tests/browser/3.built-bundle.test.js`), which skips when `dist/waymark.js` is missing — a fallback for direct `npx playwright test` runs, since `npm run test:browser` builds `dist/` automatically via the `pretest:browser` hook. `playwright.config.js` pins `workers: 1` and `retries: 1` because the suite includes WebGL/shader and remote-network-dependent map tests that flake under parallel workers; single-worker execution was verified fully green.

`npm run docs:sync` runs `node scripts/skill-md.js` before the contract/api checks, generating `SKILL.md` at the project root from `docs/1.api.md` (consumer API docs only).
...
- **Feature properties**: click/hover popup module in `src/geojson/createFeaturePropertiesModule.js`.
- **Type icons**: SVG sprite icon loader in `src/geojson/createIconImages.js`.
...
Waymark owns its icon set as first-party `waymark-icons` assets — there is no `@ogis/icons` git submodule.

- **Generated bundle**: `src/icons/` holds the build output — `waymark-icons.svg` (symbol sprite), `waymark-icons.woff2` (icon font), `waymark-icons.css`, `waymark-icons.scss`, `waymark-icons-variables.scss`, and `waymark-icons.json`.
- **Authored sources**: `src/icons/source/` holds the inputs — `svg/` (authored and traced SVGs), `png/` (auto-traced raster sources), and `codepoints.json` (stable glyph codepoints).
- **Naming**: the `@font-face` family and SCSS map are `waymark-icons` and `$waymark-icons-map`; CSS classes are `waymark-icon-{name}` (for example `waymark-icon-search`). In config, `config.types[typeKey].icon` names a symbol ID from the bundled sprite (for example `search`, `gear`, `position`, `speedometer2`).

To add or regenerate icons:

1. Drop new SVGs into `src/icons/source/svg/` (or PNGs into `src/icons/source/png/` for auto-tracing).
2. Run `npm run icons:build`.

The pipeline (`scripts/icons.js`) traces PNGs to SVGs, normalises every glyph to a 16×16 viewBox, builds the SVG sprite, generates the icon font plus CSS/SCSS/JSON, and writes `waymark-icons-variables.scss`. Assigned codepoints persist in `src/icons/source/codepoints.json`, so existing glyph values stay stable across rebuilds.

`src/icons/` is generated output and is excluded from Prettier via `.prettierignore` — never hand-edit it.

> [!NOTE]
> **Provenance** — the vendored icon set originates from the [OpenGIS/icons](https://github.com/OpenGIS/icons) repository (MIT licensed), itself derived from [Bootstrap Icons](https://github.com/twbs/icons) (MIT). Waymark-JS is GPL-2.0, which is compatible with MIT. The upstream set is redistributed here under the `waymark-icons` name; attribution to OpenGIS/icons and Bootstrap Icons must be preserved.
...
- `tests/browser/*.test.js` (Playwright): browser API, dev-page smoke, and built-bundle behaviour. The built-bundle test (`3.built-bundle.test.js`) serves `dist/` via `vite preview` and verifies the map loads from the built bundle with the inlined worker (no dead-worker stall) — it skips if `dist/waymark.js` is missing — a fallback for direct `npx playwright test` runs, since `npm run test:browser` builds `dist/` automatically via the `pretest:browser` hook. `1.api.test.js` also covers popup focus behaviour (opening a feature popup does not steal focus or scroll the page).
...
Browser tests capture screenshots as visual evidence of key application states. Screenshots are stored in `tests/browser/screenshots/` and are tracked in git.

### API tests (`/browser-api.html`)

| Screenshot             | What it shows                                                       |
| ---------------------- | ------------------------------------------------------------------- |
| `api-quick-start`      | Basic map instance with OpenFreeMap vector tiles, canvas rendered   |
| `api-view-mode`        | UI shell in view mode — minimal chrome, no debug panels             |
| `api-debug-mode`       | UI shell in debug mode — panels, controls, event log visible        |
| `api-raster-stacked`   | Raster basemaps stacked on top of a vector basemap with opacity     |
...
| Screenshot             | What it shows                                                       |
| ---------------------- | ------------------------------------------------------------------- |
| `dev-two-instances`    | Default dev page state — map in view mode, map-two in debug mode    |
| `dev-debug-panel`      | Debug panel visible on map-two with instance document and event log |
| `dev-basemaps-panel`   | Basemap selection panel open on both instances                      |
| `dev-geojson-uploaded` | GeoJSON file uploaded, new data layer added, map bounds fitted      |
...
- **Release** (`.github/workflows/release.yml`) — creates a git tag + GitHub Release when `package.json` version changes on `master`.
