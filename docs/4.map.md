...
- Multiple raster basemaps use top-first stack order (`raster[0]` is visually on top) and are inserted below the first `symbol` layer (or appended if no symbol layer exists).
...
- Normalised/serialised InstanceDocument basemap keys use canonical order: `raster` then `vector`.
